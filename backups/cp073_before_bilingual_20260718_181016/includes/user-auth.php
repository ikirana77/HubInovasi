...
function register_user_account(array $source): array
{
    ...
    if (mb_strlen($fullName) < 3 || mb_strlen($fullName) > 160) {
        return ['success' => false, 'message' => 'Masukkan nama penuh yang sah.'];
    }
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        return ['success' => false, 'message' => 'Masukkan alamat email yang sah.'];
    }
    if (!in_array($role, ['student', 'lecturer'], true)) {
        return ['success' => false, 'message' => 'Pilih peranan pelajar atau pensyarah.'];
    }
    if (strlen($password) < 10) {
        return ['success' => false, 'message' => 'Kata laluan mestilah sekurang-kurangnya 10 aksara.'];
    }
    if ($password !== $passwordConfirmation) {
        return ['success' => false, 'message' => 'Pengesahan kata laluan tidak sepadan.'];
    }
    ...
    try {
        ...
        return ['success' => true, 'message' => 'Pendaftaran berjaya. Akaun sedang menunggu kelulusan admin.'];
    } catch (PDOException $exception) {
        if ((string) $exception->getCode() === '23000') {
            return ['success' => false, 'message' => 'Email ini sudah didaftarkan.'];
        }
        error_log('User registration failed: ' . $exception->getMessage());
        return ['success' => false, 'message' => 'Pendaftaran tidak dapat disimpan. Sila cuba lagi.'];
    }
}
