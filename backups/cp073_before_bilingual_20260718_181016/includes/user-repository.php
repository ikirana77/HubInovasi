...
{
    return ['student' => 'Pelajar', 'lecturer' => 'Pensyarah'][$role] ?? $role;
}

function user_status_label(string $status): string
{
    return ['pending' => 'Menunggu Kelulusan', 'active' => 'Aktif', 'suspended' => 'Digantung'][$status] ?? $status;
}
...
function update_user_profile(int $userId, array $source): array
{
    ...
    if (mb_strlen($institution) > 255 || mb_strlen($programme) > 180) {
        return ['success' => false, 'message' => 'Maklumat profil terlalu panjang.'];
    }
    ...
    return ['success' => true, 'message' => 'Profil berjaya dikemas kini.'];
}
