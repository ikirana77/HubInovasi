...
function resolve_verified_google_identity(array $identity): array
{
    ...
    if (!$user) {
        ...
        if ($user) {
            ...
            if ($existingSub !== '' && !hash_equals($existingSub, $sub)) {
                return ['action' => 'error', 'user' => null, 'message' => tr('Email ini telah dipautkan kepada akaun Google yang lain.', 'This email address is linked to another Google account.'), 'identity' => null];
            }
            ...
            try {
                ...
                return ['action' => 'error', 'user' => null, 'message' => tr('Akaun Google tidak dapat dipautkan.', 'The Google account could not be linked.'), 'identity' => null];
            }
    ...
    if (!$user) {
        return ['action' => 'role_required', 'user' => null, 'message' => tr('Lengkapkan pendaftaran HubInovasi.', 'Complete your HubInovasi registration.'), 'identity' => $identity];
    }

    return match ((string) $user['account_status']) {
        'active' => ['action' => 'login', 'user' => $user, 'message' => tr('Log masuk Google berjaya.', 'Google sign-in successful.'), 'identity' => null],
        'pending' => ['action' => 'pending', 'user' => $user, 'message' => tr('Akaun anda masih menunggu kelulusan admin.', 'Your account is still awaiting administrator approval.'), 'identity' => null],
        'suspended' => ['action' => 'suspended', 'user' => $user, 'message' => tr('Akaun ini telah digantung. Hubungi admin HubInovasi.', 'This account has been suspended. Contact the HubInovasi administrator.'), 'identity' => null],
        default => ['action' => 'error', 'user' => null, 'message' => tr('Status akaun tidak sah.', 'The account status is invalid.'), 'identity' => null],
    };
}
...
function create_google_user_account(array $identity, string $role, string $institution = '', string $programme = ''): array
{
    if (!in_array($role, ['student', 'lecturer'], true)) {
        return ['success' => false, 'message' => tr('Pilih peranan pelajar atau pensyarah.', 'Select the student or lecturer role.'), 'user' => null];
    }
    $institution = trim($institution);
    $programme = trim($programme);
    if (mb_strlen($institution) > 255 || mb_strlen($programme) > 180) {
        return ['success' => false, 'message' => tr('Maklumat institusi atau program terlalu panjang.', 'The institution or programme information is too long.'), 'user' => null];
    }

    $existing = find_user_by_google_sub((string) $identity['sub']) ?: find_user_by_email((string) $identity['email']);
    if ($existing) {
        return ['success' => false, 'message' => tr('Akaun ini sudah didaftarkan. Sila log masuk semula.', 'This account is already registered. Please sign in again.'), 'user' => $existing];
    }
    ...
    try {
        ...
        return ['success' => true, 'message' => tr('Pendaftaran Google berjaya. Akaun sedang menunggu kelulusan admin.', 'Google registration successful. The account is awaiting administrator approval.'), 'user' => $user];
    } catch (PDOException $exception) {
        if ((string) $exception->getCode() === '23000') {
            return ['success' => false, 'message' => tr('Akaun Google atau email ini sudah didaftarkan.', 'This Google account or email address is already registered.'), 'user' => null];
        }
        error_log('Google user creation failed: ' . $exception->getMessage());
        return ['success' => false, 'message' => tr('Pendaftaran Google tidak dapat disimpan.', 'The Google registration could not be saved.'), 'user' => null];
    }
}
